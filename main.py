...
def hospitalgoodness(conn):
    ...
    '''
    df = pd.read_sql_query(scorequery, conn)
    if df.empty:
        return {}
...
def index():
    conn = sqlite3.connect('db.db')
    df = pd.read_sql_query("SELECT hospital_id, hospital_name, lat, lon FROM hospitals", conn)
    scores = hospitalgoodness(conn)
    conn.close()
...
def register_patient():
    if request.method == 'POST':
        ...
        conn = sqlite3.connect('db.db')
        cursor = conn.cursor()
        cursor.execute("INSERT INTO patients (patient_name, patient_age, patient_gender) VALUES (?, ?, ?)", (name, age, gender))
        conn.commit()
        conn.close()
...
def submit_prescription():
    ...
    common_drugs = cursor.execute("SELECT * FROM drugs WHERE is_broaddrug = 1").fetchall()
    specialized_drugs = cursor.execute("SELECT * FROM drugs WHERE is_broaddrug = 0").fetchall()
    conn.close()
